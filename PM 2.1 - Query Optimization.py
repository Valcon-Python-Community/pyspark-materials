...
# MAGIC %md 
# MAGIC
# MAGIC #### Use case Rob Lemmens
# MAGIC
# MAGIC Performed a function in a for loop with a cached dataframe. This was very slow and it was clear that it was reading the data from the blob storage multiple times (which was supposed to be prevented by caching)
# MAGIC
# MAGIC Solution:
# MAGIC
# MAGIC Perform an action before running the function. Which actually performs the cache!
# MAGIC
# MAGIC Explanation:
# MAGIC
# MAGIC You have transformations and actions. Transformations are not executed until an action is called. So putting a dislay() before solved the issue. 
...
spark.read.table("`data-academy`.sneaker_faqtory.fact_invoices").count()
