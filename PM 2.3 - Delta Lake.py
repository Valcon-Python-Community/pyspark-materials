...
# MAGIC %md
# MAGIC #### Nordin Bouchrit 
# MAGIC
# MAGIC - Partitioning on the State can indeed accelerate the queries that filter by state. However: 
# MAGIC   If de representing data is skewed within that column, partitioning on state might lead to an 'uneven' query performance. Try to find a balance between storage optimization within the partitioned folders and query optimization. 
# MAGIC
# MAGIC - Questions:
# MAGIC   How do you make sure the partition column that is chosen is the right one? -> show code to justify the chosen column
# MAGIC   
# MAGIC   Let's say the column is not preferred as single partition column, it might be a good idea to add another column next to 'state' as partition column: add another column to the partition that can be justified 
...
# MAGIC %md
# MAGIC
# MAGIC ## Hackathon Team 1 
# MAGIC
# MAGIC *Add questions that come up when seeing content​*
# MAGIC
# MAGIC *Example of learning goal for the specific topic​*
# MAGIC
# MAGIC *Potential features*
# MAGIC
# MAGIC *Pitfalls using the code​*
# MAGIC
# MAGIC *Write down a use case where this code would work / be used for in practice*
...
# MAGIC #### Nordin Bouchrit
# MAGIC
# MAGIC Querying older versions might take time because the historical state of the table needs to be reconstructed via the transaction log entries. Thats why we have Vacuums to limit the number of versions retained, however keep in mind that for some use cases this would only work because you can think of use cases where you want to keep a lot of history. In a use case where this would work, you might need to comply with GDPR Policy's and you can only keep user activities for 30 days. In a use case where this wouldn't work you can think of a data science project at a bank where they need to analyze potential fraudulent activity of users, in this case deleting old versions of the data might not be the best idea.
